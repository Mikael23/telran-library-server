...
import telran.library.common.BookCode;
import telran.library.common.PickCode;
import telran.library.common.RemoveBookCode;
...
EntityManager em;
    ...
    public Collection<Book> getBooksByReader(int readerId) {
	    ...
	    if (reader != null) {
	        em.refresh(reader);
	        reader.getRecords().forEach(r -> result.add(r.getBook()));
	    }
        return result;
    }
	...
	@Override
	@Transactional
    public RemoveBookCode removeBook(long isbn) {
	    RemoveBookCode result = RemoveBookCode.BOOK_NOT_FOUND;
	    Book book = getBook(isbn);
	    
	    if (book != null) {
	        em.refresh(book);
	        
	        Stream<Record> recordsStream = book.getRecords().stream();
	        
	        if (recordsStream.anyMatch(r -> r.getReturnDate() == null)) {
	            result = RemoveBookCode.BOOK_IN_USE;
	        } else {
	            em.remove(book);
	            result = RemoveBookCode.OK;
	        }
	    }
